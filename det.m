...
% title(['Trace Negative n=',num2str(n),' and d=',num2str(D(k))],'fontsize',6 )
% xlabel('alpha')
% ylabel('beta')
%  subplot(1,2,2)
...
% for i = 1 : NNODES
%     for j = 1 : 3
%         if (Tr(LNODES(i,j))<0 && Dcr(LNODES(i,j))>0 && sqrt(Dcr(LNODES(i,j)))+Tr(LNODES(i,j))<0)
...
% % ylim([0 0.2])
%  subplot(1,2,2)
%  trisurf(LNODES,x,y,abs(Tr)-sqrt(abs(Dcr)))
%  title('Trace-sqrt(Discriminant)')
%  shading interp
%  
%  Rg3 = zeros(NNODES,2);
% for i = 1 : NNODES
